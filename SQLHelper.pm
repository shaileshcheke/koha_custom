...
#get_col and filter_columns_exte added by shailesh cheke
BEGIN {
...
#added by shailesh cheke
sub filter_columns_exte
{
	my ($tablename,$searchtype)=@_;
	_filter_columns($tablename,$searchtype);
...
sub _Process_Operands{
	...
	my @tmpkeys;
	my @localkeys;
	#ADDED by shailesh cheke for image handling.
	if($field eq "image")
	{
		foreach (@$operand) {
	    push @tmpkeys, " $field = ? ";
	   # push @values, "LOAD_FILE(\"".$_."\")";
	   	my $blob_image;
	   	open image_file,"/home/shailesh/Pictures/phot.jpg";
	   	while(<image_file>)
	   	{

	   		$blob_image .= $_;
	   	}
	   	push @values, $blob_image;
